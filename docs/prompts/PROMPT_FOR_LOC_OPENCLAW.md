...
2. **Chương 3.4.2** (AI Deployment Agent): QUAN TRỌNG NHẤT - State machine, 5 skills
...
6. **Chương 4.5.3** (Kết nối Slack native plugin với OpenClaw)
...
- Triển khai: Cài đặt OpenClaw hiện có (không tự phát triển gateway), cấu hình plugin gốc `channels.slack` và tập trung vào Skills (Markdown) + Tools (Shell, Browser, File, Cron, Slack)
- Chạy 24/7, có long-term memory, tự động hoàn thành tác vụ
- Skills cần viết: deploy_decision, container_control, health_checker, rollback_handler, log_analyzer
...
2. **Chương 3.4.2**: 
   - Mô tả chi tiết State Machine (IDLE → WAITING_CONFIRMATION → DEPLOYING → HEALTH_CHECKING → ACTIVE/ROLLING_BACK)
   - Liệt kê 5 skills cụ thể và chức năng từng skill
   - Giải thích cơ chế "quyết định" của AI (khi nào cần xác nhận Slack, khi nào triển khai trực tiếp theo lệnh)
